...
#if defined(NDEBUG)
# error "Romancecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf1a778021a0be92673b621c0b838cc368f91afdee7ac23d425aa180d9f1fa281");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Romancecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Romancecoin Signed Message:\n";
...
{
    // Romancecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Romancecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Romancecoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Romancecoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Romancecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Romancecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf121066cd8fdead0bf849651bed37f3b9d81262c3f161ceeb64911208567eb99");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "My first Romance Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dff5632705236b5f07f6f2143ffc1c2142929761cd14a1badd1426a693cfa1e82087ee80b8267106983a120f7bc7a09e91e35939107702e8f7969b5ff9bc2793") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1543387029;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1311862;

        if (fTestNet)
        {
            block.nTime    = 1543386987;
            block.nNonce   = 279599;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xfe59f243fc84e671fc97e69ff8e9e83d76434ee9e8ecf3af52bf7e80b8ef5428"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Romancecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RomancecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RomancecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RomancecoinMiner(CWallet *pwallet)
{
    printf("RomancecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("romancecoin-miner");
    ...
    try { loop {
        ...
        printf("Running RomancecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RomancecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RomancecoinMiner, pwallet));
}
